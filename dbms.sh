...
function get_regex() {
    case $1 in
    CREATE)
        if [[ $2 =~ ^CREATE\ TABLE\ ([a-zA-Z0-9_]+)\ \((.*)\)[[:space:]]*(;)?$ ]]
        then
            return 0
        else
            return 1
        ...
        ;;
    INSERT)
        if [[ $2 =~ ^INSERT\ INTO\ ([a-zA-Z0-9_]+)\ \(([a-zA-Z0-9_,[:space:]]+)\)\ VALUES\ \(([^\)]+)\)*\;?$ ]]
        then
            return 0
        else
            return 1
        ...
        ;;
    SELECT)
        if [[ $2 =~ ^SELECT[[:space:]]+(\*|[a-zA-Z_][a-zA-Z0-9_]*(,[[:space:]]*[a-zA-Z_][a-zA-Z0-9_]*)*)[[:space:]]+FROM[[:space:]]+([a-zA-Z_][a-zA-Z0-9_]*)[[:space:]]*\;?$ ]]
        then
            return 0
        else
            return 1
        ...
        ;;
    UPDATE)
       if [[ $2 =~ ^UPDATE[[:space:]]+([a-zA-Z0-9_]+)[[:space:]]+SET[[:space:]]+(.+)[[:space:]]+WHERE[[:space:]]+(.+)$ ]] 
        then
            return 0
        else
            return 1
...
function table_menu() {
    ...
    echo "   Example: CREATE TABLE users (id:INTEGER PRIMARY KEY, name:TEXT, salary:FLOAT)"
    echo ""
    ...
    if [ $? -eq 0 ]
     then
        ...
        for column in "${columns[@]}"; do
            ...
            # Validate datatype
            if [[ ! "$datatype" =~ ^(INTEGER|FLOAT|TEXT|DATE)$ ]]
            then
...
function insert_into_table() {
    ...
    then
        ...
        if [ "${#column_names[@]}" -ne "${#input_values[@]}" ]
        then
...
function update_table() {
    ...
    do
        ...
        if [[ "${row_data[$((where_index - 1))]}" == "$where_value" ]]
        then
            updated=true
